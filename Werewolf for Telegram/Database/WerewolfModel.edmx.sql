...
-- --------------------------------------------------
-- Date Created: 03/05/2022 09:21:40
-- Generated from EDMX file: C:\Users\Administrator\Source\Repos\Werewolf2\Werewolf for Telegram\Database\WerewolfModel.edmx
-- --------------------------------------------------
...
GO
IF OBJECT_ID(N'[dbo].[FK_GroupRanking_Group]', 'F') IS NOT NULL
    ALTER TABLE [dbo].[GroupRanking] DROP CONSTRAINT [FK_GroupRanking_Group];
GO
...
CREATE TABLE [dbo].[GlobalStats] (
    ...
    [MostKilledFirstNightId] bigint  NULL,
    [MostLynchedFirstDayId] bigint  NULL,
    [MostKilledFirstDayId] bigint  NULL,
    [BestSurvivorId] bigint  NULL
);
GO
...
-- Creating foreign key on [GroupId] in table 'GroupRanking'
ALTER TABLE [dbo].[GroupRanking]
ADD CONSTRAINT [FK_GroupRanking_Group]
    FOREIGN KEY ([GroupId])
    REFERENCES [dbo].[Groups]
        ([Id])
    ON DELETE NO ACTION ON UPDATE NO ACTION;
...
-- Creating non-clustered index for FOREIGN KEY 'FK_GroupRanking_Group'
CREATE INDEX [IX_FK_GroupRanking_Group]
ON [dbo].[GroupRanking]
    ([GroupId]);
GO
